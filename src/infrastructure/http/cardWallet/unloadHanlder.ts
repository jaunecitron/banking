...
const validateParametersMiddleware = async (ctx: Context, next: Next): Promise<void> => {
  ...
  if (!validated) {
    ctx.throw(400, 'Invalid parameters', {
      code: 'UNLOAD_CARD_FROM_WALLET_INVALID_PARAMETERS',
...
const validateRequestBodyMiddleware = async (ctx: Context, next: Next): Promise<void> => {
  ...
  if (!validated) {
    ctx.throw(400, 'Invalid payload', {
      code: 'UNLOAD_CARD_FROM_WALLET_INVALID_PAYLOAD',
...
const HttpHandler = (cardWalletRepository: CardWalletService) => async (ctx: Context): Promise<void> => {
  const { userId, companyId, cardId, amount } = ctx.state;
  let cardWallet;
  try {
    cardWallet = await cardWalletRepository.unloadCard(userId, companyId, cardId, amount);
  } catch (err) {
    switch (err.constructor) {
      case CardNotFound:
        ctx.throw(404, err.message, { code: 'UNLOAD_CARD_FROM_WALLET_CARD_NOT_FOUND', details: err.message });
        break;
      case WalletNotFound:
        ctx.throw(404, err.message, { code: 'UNLOAD_CARD_FROM_WALLET_WALLET_NOT_FOUND', details: err.message });
        break;
      case ConvertionRateUnavailable:
        ctx.throw(503, err.message, { code: 'UNLOAD_CARD_FROM_WALLET_CONVERSION_UNAVAILABLE', details: err.details });
        break;
      default:
        throw err;
    }
...
export const UnloadCardHttpHandler = (cardWalletRepository: CardWalletService): ComposedMiddleware<Context> =>
  compose([
